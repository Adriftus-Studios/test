Banner_Designer_Config:
  ...
  Modes:
    Test:
      subtitle: <aqua>in <bold>Testing Mode
  Signs:
    Color:
      ...
      - if <context.location.above.material.name> == diamond_block:
        - if <player.is_sneaking>:
          - if <player.has_flag[banner_designer_createready]>:
            ...
            - adjust <[layerdownbutton].above[3]> sign_contents:<script[Banner_Designer_Config].parsed_key[Signs.Color.default]>
            - adjust <[layerupbutton].above[3]> sign_contents:<script[Banner_Designer_Config].parsed_key[Signs.Pattern.default]>
            - adjust <[completebutton].below[3]> sign_contents:<script[Banner_Designer_Config].parsed_key[Signs.Layer.default]>
...
Banner_Designer_Function:
  ...
  debug: true
  events:
    on player right clicks observer:
      - if <context.location.cuboids.contains_any_text[banner_designer_]||false>:
        - if !<player.has_flag[banner_machine_in_use]>:
          - flag <player> banner_machine_in_use
          - define uuid <context.location.cuboids.get[1].after_last[_]>
      ...
      - if <context.location.cuboids.contains_any_text[banner_designer_]||false>:
        ...
        - if <player.has_flag[banner_machine_in_use]>:
          ...
          - choose <context.location.after_last[_]>:
            ...
            - case layerup:
              - if <player.flag[banner_designer.<[uuid]>.layer]||0> == <script[Banner_Designer_Config].data_key[Max_Layers]>:
                - stop
              - else:
                - flag player banner_designer.<[uuid]>.layer:++
              - wait 1t
        ...
        - choose <context.entity.flag[banner_designer.<[uuid]>]>:
          ...
          - case patterndown:
            ...
            - if <player.flag[banner_designer.<[uuid]>.pattern.<[layer]>]||0> == 0:
              - flag <player> banner_designer.<[uuid]>.pattern.<[layer]>:38
            - else:
              - flag <player> banner_designer.<[uuid]>.pattern.<[layer]>:--
          - case exit:
            - if <player.has_flag[banner_designer_exitready]>:
              - flag <player> banner_designer_exitready:!
              - title "title:<dark_red>Exiting Banner Machine" "subtitle:<red>Your token has been refunded." targets:<player>
            ...
            - else:
              ...
              - stop
          - case reset:
            - if <player.has_flag[banner_designer.<[uuid]>.layer]> || <player.has_flag[banner_designer.<[uuid]>.color]>:
          ...
          - default:
            - stop
        - define base_color <script[Banner_Designer_Config].data_key[Color.<player.flag[banner_designer.<[uuid]>.color.0]||0>]>
        - repeat <script[Banner_Designer_Config].data_key[Max_Layers]>:
          - if <player.flag[banner_designer.<[uuid]>.pattern.<[value]>]||0> > 0:
            - define banner_layers:->:<script[Banner_Designer_Config].data_key[Color.<player.flag[banner_designer.<[uuid]>.color.<[value]>]>]||white>/<script[Banner_Designer_Config].data_key[Pattern.<player.flag[banner_designer.<[uuid]>.pattern.<[value]>]>.internal]>
        - if <[banner_layers]||null> != null:
          - adjust <player.flag[banner_designer.<[uuid]>.armor_stand].as_entity> equipment:<list[air|air|air|<[base_color]>_banner[patterns=<[banner_layers]>]]>
        - else:
          - adjust <player.flag[banner_designer.<[uuid]>.armor_stand].as_entity> equipment:<list[air|air|air|<[base_color]>_banner]>
        - inject Banner_Designer_Update instantly
...
Banner_Designer_Update:
  ...
  script:
    ...
    - define color "<script[Banner_Designer_Config].data_key[Color.<player.flag[banner_designer.<[uuid]>.color.<player.flag[banner_designer.<[uuid]>.layer]||0>]||0>].replace[_].with[ ]>"
    - define pattern <script[Banner_Designer_Config].data_key[Pattern.<player.flag[banner_designer.<[uuid]>.pattern.<player.flag[banner_designer.<[uuid]>.layer]>]||0>.name]>
...
Banner_Designer_Stop:
  type: task
  debug: true
  script:
  - if !<player.flag[banner_designer.<[uuid]>].is_empty||false>:
    - inject Banner_Designer_Update.stop instantly
