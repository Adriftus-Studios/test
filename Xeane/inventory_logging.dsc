  ...
  script:
    ...
    - foreach <[list]> as:map:
      - if <[map].get[uuid]> = <[uuid]>:
        ...
        - define cause <[map].get[cause]>

    # Restore Inventory Button
    - inventory set slot:50 d:<[inventory]> "o:player_head[skull_skin=<[target].skull_skin>;display=<&a>Restore Inventory;lore=<&a>Requires Empty Inventory;flag=cause:<[cause]>;flag=run_script:inventory_logger_view_inventory_restore;flag=uuid:<[map].get[uuid]>;flag=target:<[target]>]"
