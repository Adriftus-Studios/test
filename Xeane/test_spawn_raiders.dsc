test_spawn_blood_raiders:
  ...
  script:
    ...
    - foreach <proc[define_curve1].context[<player.location>|<[location]>|4|-90|0.5]>:
      - playeffect redstone quantity:10 special_data:10|#660000 offset:0.2 at:<[value]> targets:<server.online_players>
      - wait 1t
    - repeat 80:
      - playeffect redstone quantity:40 special_data:10|#660000 offset:0.7 at:<[location]> targets:<server.online_players>
      - wait 2t
...
test_spawn_blood_raiders_task:
  ...
  script:
    ...
    - foreach <[locations]>:
      - playeffect redstone quantity:10 special_data:5|#660000 offset:0.2 at:<[value]> targets:<server.online_players>
      - wait 2t
    - repeat 3:
      - playeffect redstone quantity:10 special_data:5|#660000 offset:0.7 at:<[spawn_at].above> targets:<server.online_players>
      - wait 2t
