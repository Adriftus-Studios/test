...
airship_move:
  ...
  script:
    ...
    - title title:<&f><&font[adriftus:overlay]><&chr[1004]><&chr[F801]><&chr[1004]> fade_in:5t stay:<[schematic_count].mul[2].add[10]>t fade_out:1.5s targets:<[target_players]>
    - wait 5t
    ...
    # Move Bed Spawns
    - if <server.has_flag[airships.ship.<[id]>.bed_spawns]>:
      - foreach <server.flag[airships.ship.<[id]>.bed_spawns]> key:target as:offset:
        - if <[target].as[player].bed_spawn.distance[<[current_location].add[<[offset]>]>]> < 2.75:
          - adjust <[target].as[player]> bed_spawn_location:<[new_location].add[<[offset]>]>
        - else:
          - flag server airships.ship.<[id]>.bed_spawns.<[target]>:!
      - wait 1t
    ...
    - wait 1t
    - define targets <cuboid[airship_<[id]>].entities[!player]>
    - adjust <[targets]> gravity:false
    ...
    - foreach <server.flag[airships.ship.<[id]>.chunks]>:
      - chunkload <[value]> duration:10s
      - wait 2t
    - title title:<&f><&font[adriftus:overlay]><&chr[1004]><&chr[F802]><&chr[1004]> fade_in:5t stay:<[old_cuboid].chunks.size.mul[3].add[5]>t fade_out:1.5s targets:<[current_location].find_players_within[140]>
    - wait 4t
    - foreach <[old_cuboid].chunks>:
      - define cuboid <[value].cuboid>
      - define mini_cuboid <[cuboid].min.with_y[<[final_low_y]>].to_cuboid[<[cuboid].max.with_y[<[final_high_y]>]>]>
      - define delete_blocks <[mini_cuboid].blocks[rail|ladder|*_button|sea_pickle|carrots|*_carpet|*torch|lever|tripwire_hook|*_bed|lantern|*sign|bell|*azalea|*_door|*_pressure_plate|*_banner]>
      - wait 1t
      - ~modifyblock <[delete_blocks]> air no_physics delayed
      - wait 1t
      - define mini_cuboid <[cuboid].min.with_y[<[final_low_y]>].to_cuboid[<[cuboid].max.with_y[<[final_high_y]>]>]>
...
airship_create_elevators:
  ...
  script:
    ...
    - run airship_elevator_particles def:<[id]> if:<server.flag[airships.ship.<[id]>.elevator_status]>
