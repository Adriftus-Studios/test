...
grappling_hook_shoot:
  ...
  script:
    ...
    - define target <player.location.precise_cursor_on_block[<[range]>].add[<player.eye_location.precise_impact_normal[<[range]>]>].if_null[null]>
    - if <[target]> == null:
    ...
    - if !<entry[ent].spawned_entity.is_spawned>:
      ...
      - stop
    - wait 1t
    - adjust <entry[ent].spawned_entity> velocity:<[target].sub[<player.eye_location>].normalize>
