admin_item:
  ...
  script:
    ...
    - if <[item]> == null:
      ...
      - stop
    - repeat 20:
      - playeffect effect:squid_ink at:<player.location.above[3]> quantity:20 offset:0.5
      - wait 2t
    - spawn dropped_item[item=<[item]>] <player.location.above[3]> save:item
    - shoot <entry[item].spawned_entity> destination:<player.location.forward_flat[4]> origin:<player.location.above[3]>
    - repeat 20:
      - playeffect effect:squid_ink at:<entry[item].spawned_entity.location.below[0.5]> quantity:20 offset:0.5
      - wait 1t