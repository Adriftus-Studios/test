...
level_design_open_main_menu:
  ...
  script:
    ...
    - foreach <[settings]>:
       - define lore:|:<list[<&b><&l>Triggers<&co>].include[<[location].flag[level_design.settings.<[value]>.triggers].keys.parse[data_key[display]]>].include[<&a>|<&a>]>
       - define lore:|:<list[<&b><&l>Effects<&co>].include[<[location].flag[level_design.settings.<[value]>.effects].keys.parse[data_key[display]]>].include[<&a>|<&a>]>
...
level_design_open_setting_menu:
  ...
  script:
    ...
    - foreach <script.data_key[data.effect_slots]>:
