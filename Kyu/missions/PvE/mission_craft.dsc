...
mission_craft_assignment:
  ...
  script:
    ...
    - define map <[map].with[name].as[<[config].data_key[description].first.replace[Items].with[<[name]>]>]>
    - define map <[map].with[description].as[<[config].data_key[name].replace[X].with[<[map].get[max]>].replace[items].with[<[name]>]>]>
